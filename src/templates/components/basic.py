...
def header(page: ft.Page, title: str):
    return ft.Container(
        content=ft.Row(
            controls=[
                ft.Text(title, size=30, color=ft.Colors.WHITE, weight="bold", text_align=ft.TextAlign.CENTER),
            ],
            wrap=True,
        ),
...
def home_apps_lv(page: ft.Page):
    home_apps_lv = ft.ListView(expand=True, spacing=10)
    for app_name, _ in page.home.items():
        home_apps_lv.controls.append(
            ft.ElevatedButton(
                text=app_name,
                on_click=lambda e: page.go(f"/home/{app_name}"),
            )
        )
    return home_apps_lv
